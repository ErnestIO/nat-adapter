...
func TestBasicRedirections(t *testing.T) {
	...
	Convey("Given this service is fully set up", t, func() {
		...
		n.Subscribe("config.get.connectors", func(msg *nats.Msg) {
			n.Publish(msg.Reply, []byte(`{"nats":["fake","vcloud","vcloud-fake","aws","aws-fake"]}`))
		})

		setup()

		n.Subscribe("nat.create.fake", func(msg *nats.Msg) {
			chfak <- true
		})
		n.Subscribe("nat.create.error", func(msg *nats.Msg) {
			cherr <- true
		})
		n.Subscribe("nat.create.vcloud", func(msg *nats.Msg) {
			chvcl <- true
		})
		Convey("When it receives an invalid fake message", func() {
			n.Publish("nat.create", []byte(`{"service":"aaa"}`))
			Convey("Then it should redirect it to a nat error creation", func() {
				So(wait(cherr), ShouldBeNil)
			})
		})
		Convey("When it receives a valid fake message", func() {
			n.Publish("nat.create", []byte(`{"service":"aaa","router_type":"fake"}`))
		...
		Convey("When it receives a valid vcloud message", func() {
			n.Publish("nat.create", []byte(`{"service":"aaa","router_type":"vcloud"}`))
			Convey("Then it should redirect it to a fake connector", func() {
